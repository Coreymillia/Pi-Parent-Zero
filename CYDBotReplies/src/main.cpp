/**
 * PiParent CYD Display — Split-screen dashboard + per-device row pages (NON-INVERTED)
 *
...
// ---- Display (CYD standard — hardware SPI, non-inverted) ----
#define GFX_BL 21
...
static void buildPages() {
    ...
    for (int i = 0; i < bm_count; i++) {
        ...
        int pi = -1;
        for (int j = 1; j < page_count; j++) {  // start at 1, skip DASHBOARD
...
static void drawRows() {
    ...
    if (page_count <= 1) {  // only dashboard, no device pages
        gfx->setTextSize(1);
        gfx->setTextColor(COL_GREY, COL_BG);
